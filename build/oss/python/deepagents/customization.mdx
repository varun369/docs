...
<Warning>
Middleware implementations should never mutate any attributes post initialization.
If mutation is needed, leverage graph state instead.
Mutations at the middleware level cause subtle bugs and race conditions that are difficult to catch, because many things run concurrently (for example, subagents, parallel tools, and parallel invocations of the same agent on different threads).
</Warning>
